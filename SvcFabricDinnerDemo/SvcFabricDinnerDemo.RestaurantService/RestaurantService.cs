...
namespace SvcFabricDinnerDemo.RestaurantService
{
    ...
    /// </summary>
    internal sealed class RestaurantService : StatefulService, IService, IRestaurantService, IRestaurantAdminService
    {
        ...
        async Task<IList<RestaurantTable>> IRestaurantService.GetTablesOfRestaurant(Guid restaurantId)
        {
            using (var tx = this.StateManager.CreateTransaction())
            {
                var dic =
